...
import { Timer, CalendarDays, Target, CheckCircle, Copy } from "lucide-react";
import { format } from "date-fns";
import { fr } from "date-fns/locale";
import { useToast } from "@/hooks/use-toast";
import { AgeResult } from "../types";

interface AgeCalculatorTabProps {
  birthDate: string;
  setBirthDate: (date: string) => void;
  calculateAge: () => AgeResult | string;
}

export const AgeCalculatorTab = ({
  birthDate,
  setBirthDate,
  calculateAge
}: AgeCalculatorTabProps) => {
            ...
            <div className="p-4 lg:p-6 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/40 dark:to-amber-900/40 rounded-2xl border-2 border-orange-200 dark:border-orange-700 shadow-inner">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => copyToClipboard(age.primary)}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg">
                  ...
                  <p className="text-orange-600 dark:text-orange-400 text-sm lg:text-base">{age.nextBirthday}</p>
                </div>
