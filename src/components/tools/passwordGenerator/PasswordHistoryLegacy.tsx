...
interface PasswordHistoryLegacyProps {
  history: LegacyPasswordEntry[];
  templates?: LegacyTemplate[];
  onCopy: (password: string, entryId?: string) => void;
  onToggleFavorite: (entryId: string) => void;
}

export const PasswordHistoryLegacy = ({
  history,
  templates = [],
  onCopy,
  onToggleFavorite
}: PasswordHistoryLegacyProps) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterBy, setFilterBy] = useState<'all' | 'favorites' | 'strong'>('all');

  const getTemplateName = (entry: LegacyPasswordEntry) => {
    if (entry.templateId) {
  ...
  const filteredHistory = history.filter(entry => {
    const matchesSearch = searchTerm === '' || 
      ...
      getTemplateName(entry).toLowerCase().includes(searchTerm.toLowerCase());
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs text-gray-500">
                      ...
                      <span>• {entry.settings?.length || 'N/A'} caractères</span>
                      <span>• {getTemplateName(entry)}</span>
                      {entry.copyCount && entry.copyCount > 0 && (
